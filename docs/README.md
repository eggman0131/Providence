...
| **Exploration / prototype** (`explore/*` probe, never merged) | `10-game-design.md` for the mechanic; the contract still applies, but I1/I6 are relaxed and only `cargo xtask explore` runs ([ADR 0016](./decisions/0016-exploration-lane-and-subsystem-isolation.md)) | `20`, `50`, `60`, `contracts/`; skip coverage/determinism/schema |
...
Documentation and environment stack decided (ADRs [0006](./decisions/0006-rust-language-and-runtime.md)–[0010](./decisions/0010-branch-workflow-and-ci.md)), and **Phase 1 (contract §7.1–7.2) is built**: the Cargo workspace realises the architecture as a crate graph, and the one-command gate — `cargo gate` — runs format, clippy, boundary, magic-number, schema-drift, config-validity, key-integrity, cargo-deny, and tests+coverage checks, green on the placeholder codebase. CI runs the same gate on every PR (ADR 0010). A fast **exploration lane** — `cargo xtask explore` (format + clippy only) — now exists for throwaway `explore/*` probes ([ADR 0016](./decisions/0016-exploration-lane-and-subsystem-isolation.md)); it is a prototyping aid, **not** a Definition of Done. Next per §7.4: the config/parameter layer (Phase 2), then the deterministic core (Phase 3).

**Planning & backlog live on GitHub** ([ADR 0012](./decisions/0012-project-management-on-github.md)): tasks and bugs are [Issues](https://github.com/eggman0131/providence/issues), the roadmap / "what's next" is the [Project board](https://github.com/eggman0131/providence/projects), and *decisions* remain ADRs here. Both formerly-open `adr-needed` items are now decided — **LLM runtime & model** ([#8](https://github.com/eggman0131/providence-legacy/issues/8)) by [ADR 0014](./decisions/0014-ollama-local-llm-runtime.md), and the **debug/HUD UI** ([#9](https://github.com/eggman0131/providence-legacy/issues/9)) by [ADR 0015](./decisions/0015-debug-hud-ui-layer.md).
