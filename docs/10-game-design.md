...
- The cascade is **naturally bounded** by `sim.terrain.max_height` (a cone cannot rise past the world maximum). **Immovable** features can forbid an op: terrain shaping must not silently destroy what another subsystem owns ([ADR 0016](./decisions/0016-exploration-lane-and-subsystem-isolation.md); model in [ADR 0017](./decisions/0017-vertex-heightfield-terrain.md)). Terrain-owned immovables — **rock and trees**, scattered by worldgen at `content.terrain.rock`/`tree.density_permille` — are live: a raise/lower whose cascade would move one is **refused whole**. Cross-subsystem immovables (opponent buildings) stay parked.
