...
| `RendererPort` | Present a derived **`TerrainFrame`** snapshot — grid dims, row-major heights, and (grown per [ADR 0023](./decisions/0023-derived-terrain-snapshot-per-vertex-type.md)) row-major per-vertex terrain **types**; all *derived* read-only data, no simulation or camera/view state ([ADR 0020](./decisions/0020-workbench-runtime-and-rendererport.md)). A frame built only to read heights (picking) carries an empty `types` slice. The camera is adapter-local view state and never crosses the boundary. | on-screen renderer (`wgpu`/Metal, [ADR 0007](./decisions/0007-wgpu-rendering-framework.md)); headless render-to-PNG (agent visual self-check); no-op test double. |
