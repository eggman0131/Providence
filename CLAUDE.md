...
| `providence-ports` | [`crates/ports`](crates/ports) | Port **interfaces** for every side effect (LLM, render, input, persistence, clock, RNG, …) plus the DTOs they hand across (`RendererPort` + the `TerrainFrame` snapshot — grown with a per-vertex terrain-**type** channel, ADR 0020/0023; the `SimDriver` interactive seam + the `TerrainCommand` shaping DTO, ADR 0022). A zero-dep `no_std` leaf; the core and every adapter depend on it, never the reverse. |
